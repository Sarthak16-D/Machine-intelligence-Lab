...
class Tensor:
    ...
    def grad_add(self, gradients=None):
        ...
        self.history[1].grad=np.zeros_like(self.history[1].arr)
        if self.history[1].requires_grad: self.history[1].grad+= np.ones_like(self.history[1].arr)
        self.history[2].grad=np.zeros_like(self.history[2].arr)
        if self.history[2].requires_grad: self.history[2].grad+=np.ones_like(self.history[2].arr)
        if gradients is None:
            return (self.history[1].grad,self.history[2].grad)
        if self.history[1].requires_grad: 
            self.history[1].grad = np.multiply(np.ones_like(self.history[1].arr),gradients)
        if self.history[2].requires_grad: 
            self.history[2].grad = np.multiply(np.ones_like(self.history[2].arr),gradients)
        return (self.history[1].grad, self.history[2].grad)
    ...
    def grad_matmul(self, gradients=None):
        ...
        if gradients != None:
        	if self.history[1].requires_grad:
        		self.history[1].grad += np.multiply(np.matmul(np.ones_like(self.history[1].arr), self.history[2].arr.transpose()), gradients)
        	if self.history[2].requires_grad:
        		self.history[2].grad += np.multiply(np.matmul(np.ones_like(self.history[2].arr), self.history[1].arr).transpose(), gradients)
        else:
        	if self.history[1].requires_grad:
        		self.history[1].grad += np.matmul(np.ones_like(self.history[1].arr), self.history[2].arr.transpose())
        	if self.history[2].requires_grad:
        		self.history[2].grad += (np.matmul(np.ones_like(self.history[2].arr), self.history[1].arr)).transpose()
    ...
    def backward(self, gradients=None):
        ...
        #     print(gradients)
        # else:
        #     self.backward[self.history[1]]
        #     self.backward[self.history[2]]
