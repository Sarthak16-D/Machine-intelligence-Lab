...
class KNN:
    ...
    def find_distance(self, x):
        ...
        for i in range(len(x)):
            ...
            for j in range(len(self.data)):
                temp.append(minkowski_dist(x[i], self.data[j], self.p))
            dist.append(temp)
        # print(dist)
        return dist
    ...
    def k_neighbours(self, x):
        ...
        for i in range(len(dp)):
            ...
            if self.weighted:
                for e in temp:
                    e=1/((e*sum(temp))+0.0000000001)
            temp = sorted(temp)
            knl.append(temp[:self.k_neigh])
        for i in range(len(knl)):
            il = [dp[i].index(j) for j in knl[i]]
            idxl.append(il)
        return (knl, idxl)
    ...
    def predict(self, x):
        ...
        """
        # TODO
        kne = self.k_neighbours(x)
        pred = []
        for i in kne[1]:
            p = []
            for j in i:
                p.append(self.target[j])
            pred.append(statistics.mode(p))
        return pred
